    ...
    >
      ...
      >
        <Image
          ...
          width={200}
          height={200}
          className="mx-auto h-[82px] w-[82px] sm:h-[118px] sm:w-[118px] md:h-[160px] md:w-[160px] lg:h-[184px] lg:w-[184px]"
          priority
        />
      </motion.div>
      ...
      <motion.div
        variants={itemVariants}
        className="relative w-full max-w-[var(--content-narrow)] rounded-[2rem] border border-oakridge-light-teal/28 bg-oakridge-navy/90 px-4 py-6 sm:rounded-[2.5rem] sm:px-8 sm:py-9 md:px-12 md:py-12"
      >
        {/* Floating Chapter Badge */}
        <motion.div
          variants={badgeVariants}
          className="absolute right-2 -top-3 z-20 rounded-lg border-2 border-oakridge-gold bg-oakridge-warm-white px-3 py-1 text-sm font-black text-oakridge-navy sm:right-[8%] sm:-top-6 sm:rounded-xl sm:px-4 sm:py-1.5 sm:text-2xl md:right-[10%] md:-top-8 md:px-5 md:py-2 md:text-4xl"
        >
        ...
        <h1 className="font-heading text-[clamp(2.15rem,8vw,8rem)] font-black leading-[1.03] tracking-[-0.045em] text-oakridge-warm-white">
          <span className="block">{HERO_CONFIG.heading.line1}</span>
          <span className="block">
            <span className="text-oakridge-gold">{HERO_CONFIG.heading.highlight}</span>{" "}
            {HERO_CONFIG.heading.line2}
          </span>
          <span className="block">{HERO_CONFIG.heading.line3}</span>
        </h1>
      </motion.div>
        ...
        {HERO_CONFIG.buttons.map((btn) => (
          <Link
            ...
            className={`min-w-[210px] rounded-full px-5 py-3 text-[11px] font-bold uppercase tracking-[0.1em] transition-all duration-300 sm:min-w-0 sm:px-8 sm:py-4 sm:text-[13px] md:px-9 ${
              btn.style === "primary"
                ? "border border-oakridge-gold bg-oakridge-gold text-oakridge-navy hover:bg-oakridge-warm-white"
      ...
      <motion.div
        variants={itemVariants}
        className="w-full max-w-[var(--content-narrow)] rounded-[1.8rem] border border-oakridge-light-teal/24 bg-oakridge-navy/88 px-4 py-4 sm:mt-2 sm:rounded-[2rem] sm:px-6 sm:py-5"
      >
